...
export function ModelSelector({ compact = false, onAuthRequired, onManageModels }: ModelSelectorProps) {
  ...
  const { models, selectedModel, keysError, isStreaming } = chatState;
  ...
  const modelsByProvider = models.reduce((acc: Record<string, ModelOption[]>, m: ModelOption) => {
                      ...
                      {hoveredProvider === provider && (
                        ...
                        >
                          {providerModels.map((m) => (
                            <button
                              key={m.ref}
                              onClick={(e: React.MouseEvent<HTMLButtonElement>) => { e.stopPropagation(); handleSelectModel(m.ref); }}
